...
def verify(m, public_key, signed_message):
    ...
    message = encode_defunct(text=m)  # Encode the message
    signer = eth_account.Account.sign_message(message, signature=signed_message.signature)  # Verify the message
    valid_signature = signer  # True if message verifies, False if message does not verify
