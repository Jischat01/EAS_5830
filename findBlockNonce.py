...
def mine_block(k, prev_hash, transactions):
    ...
    tx_bytes = ''.join(transactions).encode('utf-8')
    base = prev_hash + tx_bytes

    nonce_int = 0
    while True:
        nonce = str(nonce_int).encode('utf-8')
        hash_bytes = hashlib.sha256(base + nonce).digest()
        hash_bin = bin(int.from_bytes(hash_bytes, byteorder='big'))[2:].zfill(256)
