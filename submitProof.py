...
def merkle_assignment():
    ...
    random_leaf_index = random.randint(10, len(leaves)-1) #TODO generate a random index from primes to claim (0 is already claimed)
...
def prove_merkle(merkle_tree, random_indx):
    ...
    for level in merkle_tree[:-1]:
        pair_idx = idx ^ 1  # sibling index
        if pair_idx < len(level):
            merkle_proof.append(level[pair_idx])
        idx //= 2
